...
if (isset($_POST['daftar'])) {
    ...
    if (empty($nama) || empty($phone_number) || empty($email) || empty($alamat) || empty($semester) || empty($ipk) || empty($beasiswa) || empty($berkas)) {
        ...
        if (in_array($get_ext, $ext_file_required) === true) {
            ...
            $query = mysqli_query($conn, "INSERT INTO mahasiswa VALUES('$convertLowerCase','$nim','$nama','$alamat','$phone_number','$semester','$ipk', '$beasiswa', '$berkasName','$email', 'Belum di Verifikasi')");
            if ($query) {
